...
def test_pipeline_health_written(tmp_path, monkeypatch):
    ...
    cfg = {
        ...
        "data": {"quality": {"min_coverage": 0.0}},
        "regime": {},
        "strategy": {"entries_5m": {}},
        "backtest": {"initial_capital": 10000},
        "features": {"ofi": {"enabled": True, "levels": [1]}, "kyle_lambda": {"enabled": False}, "replenishment": {"enabled": False}},
        "hazard": {"horizon_minutes": 5, "feature_window_minutes": 3},
        "labeling": {"triple_barrier": {"price_source": "mid_close", "vol": {"kind": "ewma", "window_1m_bars": 5, "min_sigma": 1.0e-6}, "barriers": {"sl_mult": 1.0}}},
        "model": {"training": {"validation": {"splits": {"train_days": 1, "test_days": 1, "step_days": 1}}}, "preprocessing": {}},
        "policy": {"exit": {"enabled": False}, "fail_fast": {"enabled": False}, "add_risk": {"enabled": False}},
    }
    ...
    health_path = artifacts_dir / "pipeline_health.json"
    assert health_path.exists()
    payload = json.loads(health_path.read_text(encoding="utf-8"))
    assert "agg_trades" in payload
